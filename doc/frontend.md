...
### 文件命名
文件名称以及文件后缀中的 **英文字母统一使用小写** , 如 `bg.png`, `index.html`, `app.js`等.
...
### 通用规范
#### 缩进
代码文件统一使用4个空格来作为一次缩进, 不要使用tab或者tab与空格混用, 以保证代码在不同环境下显示一致.

可在代码编辑器的配置中将一个tab的大小设置为4个空格.
#### 注释
...
### HTML部分
#### 文档类型  
推荐使用HTML5文档类型 `<!DOCTYPE html>`,能够确保在每个浏览器中拥有一致的展现。
...
#### 移动端显示
```html
<meta name="viewport" content="width=device-width, initial-scale=1.0, maxinum-scale=1.0, mininum-scale=1.0, user-scalable=0">
```
...
#### 缩进,分号,单行长度
* 每条语句之后一律以分号结束，不可以省略
* 单行长度，理论上不要超过80列, 方便阅读，超过80个字符的字符串应该使用字符串连接换行
